 ...
 * 3. Budget optimizer -- best pick per day within budget
 */
...
function preFilterMenu(menuItems: MenuItem[]): MenuItem[] {
...
function mergeScores(nutritionScores: ScoreEntry[], longevityScores: ScoreEntry[]): Record<string, { nutrition?: number; longevity?: number }> {
  const map: Record<string, { nutrition?: number; longevity?: number }> = {};
...
function scoreFilter(
  items: MenuItem[],
  mergedScores: Record<string, { nutrition?: number; longevity?: number }>,
  nutritionScores: ScoreEntry[],
  longevityScores: ScoreEntry[]
) {
  ...
  for (const item of items) {
    ...
    if (!scores) {
      ...
      const nutritionOk = !('nutrition' in scores) || (scores.nutrition || 0) >= MIN_ITEM_SCORE;
      const longevityOk = !('longevity' in scores) || (scores.longevity || 0) >= MIN_ITEM_SCORE;
...
function formatMenuForPrompt(items: MenuItem[]): string {
  const byDay: Record<string, MenuItem[]> = {};
  for (const item of items) {
 ...
 */
export async function analyzeMenu(menuItems: MenuItem[], budget = 20): Promise<Recommendation[]> {
...
Output ONLY valid JSON, no other text.`,
  ...
  const nutritionScores = parseJsonArray(
    (nutritionResponse.content[0] as Anthropic.TextBlock).text
  ) as ScoreEntry[];
...
Output ONLY valid JSON, no other text.`,
  ...
  const longevityScores = parseJsonArray(
    (longevityResponse.content[0] as Anthropic.TextBlock).text
  ) as ScoreEntry[];
...
Output ONLY valid JSON array, no other text.`,
  ...
  let recommendations = parseJsonArray(
    (optimizerResponse.content[0] as Anthropic.TextBlock).text
  ) as Recommendation[];

  // Post-safety: remove any junk items Agent 3 may have hallucinated
  ...
  // Normalize: ensure combo_json, item_name, price fields
  for (const rec of recommendations) {
    if (rec.items && Array.isArray(rec.items)) {
  ...
  // Group by day, sort by score, assign rank 1-3
  const byDay: Record<string, Recommendation[]> = {};
  for (const rec of recommendations) {
    if (!byDay[rec.day]) byDay[rec.day] = [];
    byDay[rec.day].push(rec);
  }

  // If a day lost ALL combos to budget filter, keep the cheapest
  ...
  for (const [, dayRecs] of Object.entries(byDay)) {
