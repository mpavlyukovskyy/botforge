 ...
 */
export async function sendWorkoutPrompt(ctx, chatId) {
  ensureDb(ctx.config);
...
export async function generateAdaptedWorkout(ctx, chatId, timeMinutes) {
  ...
  const exerciseHistory = (session.exercises || []).map(ex => {
    let lastPerf = null;
  ...
  const systemPrompt = `You are a personal trainer writing a workout card for Telegram (Markdown).
...
Use the lower bound of any rep range for "reps". Use the Target weight for "weight_kg". Include only exercises that appear in the final card.`;
  ...
  const userMessage = `Session: ${session.name} (${session.focus || 'general'})
...
Generate the adapted workout card.`;
  ...
  if (result.is_error || !result.text?.trim()) {
    ...
    workoutCard = [
      `*${session.name}* — Week ${program.current_week} (${timeMinutes}min)`,
      '',
      ...exerciseLines,
      adjustNote,
      '',
    ...
    const fallbackExercises = (session.exercises || []).map(ex => ({
      name: ex.name,
      sets: ex.sets,
      reps: parseInt(ex.rep_range) || 8,
      weight_kg: exerciseWeights[ex.name] || 0,
    }));
    pendingId = savePendingWorkout(ctx.config, session.name, fallbackExercises, timeMinutes);
  } else {
    ...
    if (jsonStr) {
      try {
        ...
        if (parsed.exercises?.length > 0) {
          pendingId = savePendingWorkout(ctx.config, parsed.title || session.name, parsed.exercises, timeMinutes);
        }
