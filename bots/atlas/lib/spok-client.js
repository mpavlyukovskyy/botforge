/**
 * Spok API Client — shared helpers for atlas tools
 *
 * NOT a tool (no name/execute export) — loadToolsFromDir() will skip this file.
 * Adapted from kristina/lib/atlas-client.js for Spok board sync.
 */
...
function recordFailure() {
  ...
  if (consecutiveFailures >= MAX_FAILURES) {
    ...
    console.warn(`[spok] Circuit breaker open until ${new Date(circuitOpenUntil).toISOString()}`);
  }
}

// ─── Spok HTTP ──────────────────────────────────────────────────────────────

function getSpokConfig(ctx) {
  const spok = ctx.config.integrations?.spok;
  return {
    url: (spok?.url ?? process.env.SPOK_SYNC_URL ?? 'https://spok.vc').replace(/\/$/, ''),
    endpoint: spok?.sync_endpoint ?? '/api/sync/agenda-bot/items',
    token: spok?.token ?? process.env.SPOK_SYNC_KEY,
    fundId: spok?.fund_id ?? process.env.SPOK_DEFAULT_FUND_ID,
  };
}

async function spokFetch(ctx, path, options = {}, timeoutMs = 10_000) {
  const { url, token } = getSpokConfig(ctx);
...
export async function getColumns(ctx) {
  ...
  if (isCircuitOpen()) {
    console.warn('[spok] Circuit open, returning cached columns');
    return cachedColumns;
  }

  try {
    const { fundId } = getSpokConfig(ctx);
    const params = fundId ? `?fundId=${fundId}` : '';
    const resp = await spokFetch(ctx, `/api/sync/agenda-bot/columns${params}`);
    ...
    const upsert = db.prepare(
      `INSERT OR REPLACE INTO column_cache (id, name, slug, fund_id, cached_at) VALUES (?, ?, ?, ?, datetime('now'))`
    );
    const tx = db.transaction(() => {
      for (const col of cachedColumns) {
        upsert.run(col.id, col.name, col.slug, fundId || null);
      }
    });
    tx();
...
export async function getItems(ctx, opts = {}) {
  if (isCircuitOpen()) {
    ctx.log.error('[spok] Circuit open, returning empty items');
    return [];
  }

  try {
    const { endpoint, fundId } = getSpokConfig(ctx);
    ...
    const resp = await spokFetch(ctx, path);
    ...
    ctx.log.error(`[spok] Failed to get items: ${err}`);
    recordFailure();
    return [];
  }
}
...
export async function createItem(ctx, data) {
  if (isCircuitOpen()) {
    ctx.log.warn('[spok] Circuit open, saving locally only');
    return null;
  }
  ...
  try {
    ...
    let resp = await spokFetch(ctx, endpoint, {
      method: 'POST',
    ...
    if (!resp.ok && resp.status !== 401 && resp.status !== 403) {
      ...
      ctx.log.warn(`[spok] Create failed (${resp.status}), retrying in 1s: ${text}`);
      await new Promise(r => setTimeout(r, 1000));
      resp = await spokFetch(ctx, endpoint, {
        method: 'POST',
        body: JSON.stringify(payload),
      }, fetchTimeout);
    }
    ...
    ctx.log.error(`[spok] Failed to create item: ${err}`);
    recordFailure();
    return null;
  }
}
...
export async function updateItem(ctx, itemId, data) {
  ...
  try {
    const { endpoint } = getSpokConfig(ctx);
    const resp = await spokFetch(ctx, endpoint, {
      method: 'PATCH',
    ...
    ctx.log.error(`[spok] Failed to update item: ${err}`);
    recordFailure();
    return false;
  }
}
...
export async function deleteItem(ctx, itemId) {
  ...
  try {
    const { endpoint } = getSpokConfig(ctx);
    const resp = await spokFetch(ctx, `${endpoint}?id=${itemId}`, {
      method: 'DELETE',
    });
    ...
    ctx.log.error(`[spok] Failed to delete item: ${err}`);
    recordFailure();
    return false;
  }
}
...
export async function syncAttachment(ctx, taskId, attachment) {
  ...
  try {
    ...
    ctx.log.error(`[spok] Failed to sync attachment: ${err}`);
    recordFailure();
    return false;
  }
}
...
export async function retrySyncPending(ctx) {
  ...
  const pending = db
    .prepare('SELECT id, title, column_id, assignee, deadline, status FROM tasks WHERE synced_at IS NULL')
    .all();
  ...
  for (const task of syncedTasksWithPendingSubtasks) {
    ...
    try {
      ...
      ctx.log.error(`[spok] Failed to sync subtasks: ${err}`);
      recordFailure();
    }
  }
