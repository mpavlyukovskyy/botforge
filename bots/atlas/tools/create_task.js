import { z } from 'zod';
import { getColumns, findColumnByName, createItem, updateItem, ensureDb } from '../lib/spok-client.js';
...
const createTask = {
  ...
  execute: async (args, ctx) => {
    ...
    // Save locally
    db.prepare(
      `INSERT INTO tasks (id, spok_id, title, column_name, column_id, assignee, deadline, status, synced_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`
    ).run(
      taskId,
      spokResult?.atlasId || null,
      title,
      columnName || null,
      columnId || null,
      args.assignee || null,
      args.deadline || null,
      args.done ? 'DONE' : 'OPEN',
      spokResult ? new Date().toISOString() : null,
    );

    // Mark done on Spok if requested
    if (args.done && spokResult) {
      await updateItem(ctx, spokResult.atlasId, { status: 'DONE' });
    }
    ...
    if (subtasks && subtasks.length > 0) {
      ...
      subtasks.forEach((st, idx) => {
        insertSubtask.run(taskId, st.title, idx, spokResult ? new Date().toISOString() : null);
      });
    }
    ...
    if (ctx.files && ctx.files.length > 0) {
      try {
        ...
        let synced = false;
        if (spokResult) {
          const { syncAttachment } = await import('../lib/spok-client.js');
          synced = await syncAttachment(ctx, spokResult.atlasId, {
            type: 'IMAGE',
    ...
    if (urlMatch && spokResult) {
      try {
        const { syncAttachment } = await import('../lib/spok-client.js');
        for (const url of urlMatch) {
          await syncAttachment(ctx, spokResult.atlasId, {
            type: 'LINK',
            url,
          });
        }
    ...
    if (!spokResult) result += ' (saved locally, will sync later)';
    return result;
  },
};
