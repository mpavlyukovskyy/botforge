/**
 * Atlas API Client — shared helpers for kristina tools
 *
...
export function getAtlasConfig(ctx) {
  ...
  return {
    ...
    endpoint: atlas?.sync_endpoint ?? '/api/sync/kristina-bot/items',
...
export async function getColumns(ctx) {
  ...
  try {
    const resp = await atlasFetch(ctx, '/api/sync/kristina-bot/columns');
...
function getLocalItems(ctx, opts = {}) {
  ...
  try {
    ...
    const sql = `SELECT id, spok_id, title, column_name, column_id, assignee, deadline, status, earned_status, requester, priority_tier, blocked_at, blocked_on, blocked_seconds_total
